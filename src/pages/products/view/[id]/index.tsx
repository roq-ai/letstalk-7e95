...
import { deleteStockById } from 'apiSdk/stocks';
...
function ProductViewPage() {
  ...
  const stockHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/orders/create?product_id=${data?.id}`}>
              ...
              {hasAccess('stock', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/stocks/create?product_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.stock?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/stocks/view/${record.id}`)} key={record.id}>
